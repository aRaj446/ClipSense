...
export default function SegmentScrubber({ datasets, videoUrl, variants, autoCompare }: Props) {
        ...
        compareOpen ? '' : 'hidden'
          ...
          <div className="flex items-center justify-between px-6 py-3 border-b border-slate-800 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-slate-100">Variant Comparison Editor</span>
              ...
              return (
                <div className="flex h-1.5 rounded-full overflow-hidden gap-px" title="Engagement heatmap from dataset segments">
                  {buckets.map((score, bi) => {
                    const bg = score > 0.2 ? 'bg-green-500' : score < -0.2 ? 'bg-red-500' : 'bg-slate-600'
                    return <div key={bi} className={`${bg} flex-1 h-full opacity-80`} />
                  })}
                </div>
              )
            })()}

...
import {
  ...
  RefreshCw, Square, Trash2, Sparkles,
} from 'lucide-react'
    ...
    return (
      <Card className="flex flex-col items-center py-16 text-center">
        ...
        <p className="text-slate-500 text-sm mt-1">
          Go to Upload → Smart Trailer to generate one.
        </p>
      </Card>
    )
  }
          ...
          <div key={job.id} className="border border-surface-border rounded-lg overflow-hidden">
            <div
              className={`flex items-center gap-3 px-4 py-3 bg-surface ${
                job.status === 'done' && onSelectJob ? 'cursor-pointer hover:bg-surface-card transition-colors' : ''
              }`}

...
import { isClipSenseSession, uploadToClipSense } from "./clipsense-export";
  ...
  const runExport = useCallback(
    async (
      ...
      if (finalResult?.success) {
        ...
        // Auto-upload to ClipSense if this is a ClipSense editing session
        if (isClipSenseSession()) {
          try {
            setState((prev) => ({ ...prev, phase: "Uploading to ClipSense..." }));

            // Create a buffer-based writable to capture the export as a blob
            const chunks: Uint8Array[] = [];
            const bufWritable = {
              seek() { return Promise.resolve(); },
              write(data: unknown) {
                if (data instanceof ArrayBuffer) chunks.push(new Uint8Array(data));
                else if (data instanceof Uint8Array) chunks.push(data.slice());
                else if (ArrayBuffer.isView(data)) chunks.push(new Uint8Array(data.buffer as ArrayBuffer, data.byteOffset, data.byteLength));
                return Promise.resolve();
              },
            ...
            // Re-export to buffer (same settings)
            const uploadEngine = getExportEngine();
            const gen = uploadEngine.exportVideo(project, videoSettings, bufWritable);
            let uploadResult: ExportResult | undefined;
            while (true) {
              const { value, done } = await gen.next();
              if (done) { uploadResult = value; break; }
            }

            if (uploadResult?.success && chunks.length > 0) {
              const blob = new Blob(chunks, { type: "video/mp4" });
              const uploadRes = await uploadToClipSense(blob);
              if (uploadRes.success) {
                setState((prev) => ({ ...prev, phase: "Saved & uploaded to ClipSense!" }));
              } else {
                setState((prev) => ({ ...prev, phase: "Saved! (upload failed)" }));
              }
            } else {
              setState((prev) => ({ ...prev, phase: "Saved! (upload skipped)" }));
            }
          } catch (err) {
            console.warn("[ExportRunner] ClipSense upload error:", err);
